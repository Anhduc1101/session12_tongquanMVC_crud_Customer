...
<table>
   ...
   <c:forEach items="${cus}" var="c" varStatus="loop">
       <tr>
           <td>${loop.index+1}</td>
           <td>${c.id}</td>
           <td>${c.name}</td>
           <td>${c.address}</td>
           <td>${c.email}</td>
           <td><a href="/customer/edit?idEdit=${c.id}">Edit</a></td>
           <td><a href="/customer/delete?&idDel=${c.id}" onclick="return confirm('Bạn có chắc chắn muốn xóa không?')">Delete</a></td>
       </tr>
   </c:forEach>
</table>
</body>
